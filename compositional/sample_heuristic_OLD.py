...
def estimate_label_info(label, left_quantities, right_quantities, neuron_quantities, max_size_mask):
    ...
    neuron_coverable, _ = neuron_coverable_mask_tuple
    ...
    if isinstance(label, F.Or):
        ...
        min_common_intersection = np.clip(
            min_left_common_intersection + min_right_uncovered - neuron_common, a_min=0, a_max=None
        ) # max(0, I_min^c(L) + U_min^c - N^u - N^c)
        max_common_intersection = np.minimum(
        ...
        min_common_extras = np.clip(
            min_left_common_extras + (common_space_extras - max_right_common_extras) - \
            common_space_extras,
            a_min=0, a_max=None
        )
...
def and_chain_estimation(label, concept_quantities, neuron_quantities, max_improvement, *, num_hits, max_size_mask, max_length, minimum_threshold=0):
    ...
    min_union = np.clip(neuron_sum + np.clip(
        min_common_extras +  bottom_1_common_extras - common_space_extras,
...
def comb_and_or_chain_estimation(label, concept_quantities, neuron_quantities, max_improvement, *, num_hits, max_size_mask, max_length, minimum_threshold=0):
    ...
    max_label_intersection = np.minimum(
        max_common_intersection + top_k_common_intersection, neuron_common) + \
                    + top_k_unique_intersection
    ...
    min_union = np.clip(neuron_sum + np.clip(
                                min_common_extras + bottom_1_common_extras -
                                    common_space_extras, a_min =0, a_max=max_size_mask),
