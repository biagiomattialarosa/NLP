...
def compute_activation_ranges(
        activations: torch.Tensor, num_clusters: int, quantile: float = 0.005) -> List[Tuple]:
    """Compute activation ranges for each unit.

    Args:
        activations (torch.Tensor): Activations of the unit.
        num_clusters (int): Number of clusters.
        quantile (float): Quantile to use for thresholding.

    Returns:
        activation_ranges (List[tuple]): Activation ranges for each unit.
    """
    if num_clusters == 0:
        activation_ranges = [(0, torch.tensor(float("inf")))]
    elif num_clusters == 1:
...
def compute_bitmaps(
        ...
        mask_shape: List[int], interpolation_mode='bilinear') -> torch.Tensor:
    ...
    upsampled_activations = torch.nn.functional.interpolate(
        ...
        size=mask_shape, mode=interpolation_mode)
