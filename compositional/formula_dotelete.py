...
class Leaf(F):
    ...
    """
    def __init__(self, val, neuron_mask=None, leaf_mask=None):
        self.val = val
        # We added this term to order the formulas by IoU
        self.iou = None
        self.covered = None
        self.uncovered = None
        self.indices_covered = None
        self.indices_uncovered = None
        self.max = neuron_mask.shape[1]
        # # Addition to support optimal heuristic
        # if neuron_mask is not None and leaf_mask is not None:
        #     leaf_mask = leaf_mask.bool()
        #     neuron_mask = neuron_mask.bool()
        #     self.covered = (neuron_mask & leaf_mask).sum(dim=1)
        #     self.uncovered = (neuron_mask & ~leaf_mask).sum(dim=1)
        #     self.indices_covered = self.covered.nonzero()
        #     self.indices_uncovered = self.uncovered.nonzero()
...
class BinaryNode(Node):
    ...
    def __init__(self, left, right):
        ...
        # We added this term to order the formulas by IoU
        self.iou = 0
        self.max = max(left.max, right.max)
...
class Not(UnaryNode):
    ...
    def __init__(self, leaf):
        super().__init__(leaf.val)
        self.val = leaf
        self.op = "NOT"
        self.max = leaf.max
        self.covered = leaf.uncovered
        self.uncovered = leaf.covered
        self.indices_covered = leaf.indices_uncovered
        self.indices_uncovered = leaf.indices_covered
...
class And(BinaryNode):
    ...
    def __init__(self, left, right):
        super().__init__(left, right)
        self.op = "AND"
        # if None not in [self.left.covered, self.left.uncovered, self.right.covered, self.right.uncovered]:
        #     # Optimistic estimantion (assuming full overlap)
        #     self.covered = torch.minimum(self.left.covered, self.right.covered)
        #     # AND operator cannot increase the number of covered activations
        #     self.uncovered = torch.maximum(self.left.uncovered, self.right.uncovered)
        #     self.indices_covered = self.covered.nonzero()
        #     self.indices_uncovered = self.uncovered.nonzero()
