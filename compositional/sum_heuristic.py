...
def estimate_label_info(label, left_quantities, right_quantities, neuron_quantities, max_size_mask):
    ...
    if isinstance(label, F.Or):
        ...
        min_common_intersection_sum = max(min_left_common_intersection_sum - max_right_common_intersection_sum, 0)
        ...
        min_common_extras_sum = max(min_left_common_extras_sum - max_right_common_extras_sum, 0)
        max_common_extras_sum = min(max_left_common_extras_sum ,  common_space_extras_sum- min_right_common_extras_sum) # E_max^c(L) - max(0, E_min^c(L) + E_min^c(c) - E_max^c(c))
...
def or_chain_estimation(label, concept_quantities, neuron_quantities, max_improvement, *, num_hits, max_size_mask, max_length, minimum_threshold=0):
    ...
    #(_, cumsum_unique_intersection), (_, cumsum_unique_extras) = cumsum_quantities
    ...
    min_union = num_hits + max(min_common_extras_sum + min_unique_extras_sum, bottom_1_common_extras_sum + bottom_1_unique_extras_sum) #+ max(0, bottom_1_common_extras_sum - max_common_extras_sum) + max(0, bottom_1_unique_extras_sum - max_unique_extras_sum), tot_size)
    ...
    max_union = min(num_hits + min(max_common_extras_sum + top_k_common_extras_sum, common_space_extras_sum) + min(max_unique_extras_sum + top_k_unique_extras_sum, unique_space_extras_sum), tot_size)
...
def and_not_chain_estimation(label, concept_quantities, neuron_quantities, max_improvement, *, num_hits, max_size_mask, max_length, minimum_threshold=0):
    ...
    #     last_val = label.get_vals()[0]
    #     previous_index = last_val - 1
    #     (_, cumsum_unique_intersection), (_, cumsum_unique_extras) = cum_sum_quantities
    #     if previous_index > 0:
    ...
    # Min IoU
    bottom_1_common_extras_sum = improv_common_extras[0][BOTTOM_INDEX_SUM]
    max_improv_common_extras_sum = min(max_common_extras_sum, common_space_extras_sum - bottom_1_common_extras_sum)
    min_intersection = min_unique_intersection_sum
    max_union = min(num_hits + max_unique_extras_sum + max_improv_common_extras_sum, tot_size)
